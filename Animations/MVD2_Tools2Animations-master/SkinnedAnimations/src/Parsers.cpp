...
bool Parsers::parseCollada(std::string filename, Shader* shader, GraphicsSystem& graphics_system) {
    ...
    std::string path, filename_without_path;
    
    //if position_last_separator != null,
    if (std::string::npos != position_last_separator) {
        path = filename.substr(0, position_last_separator + 1); //add relative path to path, include slash
        filename_without_path = filename.substr(position_last_separator + 1); //remove relative path from filename
    }
    ...
    //the materials system has three elements. First, assets such as images. Then nodes can reference a 'material' which references an 'effect'
    //so need to read all three and map them
    
    std::unordered_map<std::string, GLuint> imageID_engineID;
    
    XMLElement* library_images = root->FirstChildElement("library_images");
    if (library_images) {
        
        //loop all image elements
        XMLElement* image_element = library_images->FirstChildElement("image");
        for (image_element; image_element != NULL; image_element = image_element->NextSiblingElement("image")) {
            
            std::string img_id = image_element->Attribute("id");
            
            XMLElement* init_from_element = image_element->FirstChildElement("init_from");
            
            if (!init_from_element) {
                std::string err = "ERROR: Collada parser: no image found in init_from element with id " + img_id;
                print(err);
            }
            else {
                std::string image_path = path + init_from_element->GetText();
                imageID_engineID[img_id] = Parsers::parseTexture(image_path);
            }
        }
    }
    ...
    //maps sampler name to texture image name (then map to opengl id using imageID_engineID
    std::unordered_map<std::string,std::string> textureSurface_textureID;
    //then another map that maps the sampler to the surface ufff
    std::unordered_map<std::string,std::string> textureSampler_textureSurface;
    
    //so it goes: surface->sampler->image->engine_id
    ...
    if (lib_effects) {
        ...
        for (effect; effect != NULL; effect = effect->NextSiblingElement("effect")) {
            ...
            if (profile_common_element) {
                ...
                //get samplers
                XMLElement* newparam_element = profile_common_element->FirstChildElement("newparam");
                for (newparam_element; newparam_element != NULL; newparam_element = newparam_element->NextSiblingElement("newparam")) {
                    
                    std::string newparam_id = newparam_element->Attribute("sid");
                    
                    if (newparam_element->FirstChildElement("surface")){
                        std::string tex_id = newparam_element->FirstChildElement("surface")->FirstChildElement("init_from")->GetText();
                        
                        textureSurface_textureID[newparam_id] = tex_id;
                    }
                    
                    if (newparam_element->FirstChildElement("sampler2D")){
                        std::string surface_id = newparam_element->FirstChildElement("sampler2D")->FirstChildElement("source")->GetText();
                        
                        textureSampler_textureSurface[newparam_id] = surface_id;
                    }
                    ...
                    if (effect_name_element) {
                        ...
                        if (diffuse_element) {
                            ...
                            if (color_element) {
                                ...
                                new_mat.diffuse = lm::vec3(1.0f, 1.0f, 1.0f);
                            }
                            
                            //texture
                            XMLElement* texture_element = diffuse_element->FirstChildElement("texture");
                            if (texture_element) {
                                std::string texture_sampler_attr = texture_element->Attribute("texture");
                                std::string texture_surface = textureSampler_textureSurface[texture_sampler_attr];
                                std::string textureID = textureSurface_textureID[texture_surface];
                                new_mat.diffuse_map = imageID_engineID[textureID];
                                int bob = 0;
                            }
                        }
                    }
...
//
//                //copy from original indices to opengl indices
//                for (GLuint i = 0; i < num_opengl_vertices; i++){
//                    GLuint i_orig = orig_indices[i];
//                    opengl_vertex_weights_vec4[i] = orig_vertex_weights_vec4[i_orig];
//                    opengl_vertex_jointsids_string[i] = orig_vertex_jointsids_string[i_orig];
//                }
