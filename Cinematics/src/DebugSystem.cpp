...
void DebugSystem::lateInit() {
	...
	std::vector<float> temp_track_vertice1;
	std::vector<float> temp_track_vertice2;

	//TODO:
	//Upgrade the rendering of the spline
    int index = 0;
    auto& view_tracks = ECS.getAllComponents<ViewTrack>();
	
    for (auto& cc : view_tracks) {
		//We have to increase the number of knots interpolating between each of these knots 
		//We store tuples of every two knots and put 10 extra knots between them.
        ...
        if (cc.curve._knots.size() > 1)
        {
            ...
            {
				//here we inlcude the curve knots to the list of track_vertices
